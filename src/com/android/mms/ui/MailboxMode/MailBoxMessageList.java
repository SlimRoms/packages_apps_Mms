...
public class MailBoxMessageList extends ListActivity implements
    ...
    private static final String ORIGIN_PHONE_ID = "origin_phone_id";
    ...
    public void onListItemClick(ListView l, View v, int position, long id) {
        ...
        try {
            ...
            if (isDraft) {
                ...
                int phoneId = c.getInt(COLUMN_PHONE_ID);
                ...
                if (PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND == c.getInt(COLUMN_MMS_MESSAGE_TYPE)) {
                    switch (downloadStatus) {
                        ...
                        default:
                            showDownloadDialog(msgUri, phoneId);
                            break;
                    }
                } else {
    ...
    private void showDownloadDialog(final Uri uri, final int phoneId) {
        ...
        builder.setPositiveButton(R.string.yes, new OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        startDownloadAttachment(uri, phoneId);
                    }
                });
    ...
    private void startDownloadAttachment(Uri uri, int phoneId) {
        Intent intent = new Intent(this, TransactionService.class);
        intent.putExtra(TransactionBundle.URI, uri.toString());
        intent.putExtra(TransactionBundle.TRANSACTION_TYPE,
                Transaction.RETRIEVE_TRANSACTION);
        intent.putExtra(Mms.PHONE_ID, phoneId); //destination subId
        intent.putExtra(ORIGIN_PHONE_ID,
                SubscriptionManager.getPhoneId(SubscriptionManager.getDefaultDataSubId()));
        startService(intent);
    ...
    private void showSmsMessageContent(Cursor c) {
        ...
        i.putExtra("sms_phoneid", c.getInt(COLUMN_PHONE_ID));
        ...
        try {
            synchronized (mCursorLock) {
                ...
                if (mQuerySlotType == TYPE_SLOT_ONE) {
                    selStr = "phone_id = " + MessageUtils.PHONE1;
                } else if (mQuerySlotType == TYPE_SLOT_TWO) {
                    selStr = "phone_id = " + MessageUtils.PHONE2;
                }
                if (isSearchMode()) {
                    ...
                    mQueryHandler.startQuery(MESSAGE_LIST_QUERY_TOKEN, 0,
                            Uri.parse(mailboxUri), MAILBOX_PROJECTION, selStr,
